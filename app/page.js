...
// --- BIBLIOTHÈQUE D'ICÔNES (SVG Fragmentés pour éviter les erreurs de build) ---
...
export default function Home() {
  ...
  const [user, setUser] = useState(null); // Objet employé complet
  ...
  // CALCULS PODIUM ET RÔLES
  const topVendeurs = [...(data?.employees || [])].sort((a,b) => b.ca - a.ca).slice(0, 3);
  const canAccessManagement = user && ['PDG', 'Co-PDG', 'Manager'].includes(user.poste);
  ...
  return (
    <>
    <style jsx global>{`
        :root { --primary: #ff6a2b; --primary-light: #fff0eb; --bg-body: #f8f9fc; --bg-panel: #ffffff; --text-main: #1e293b; --text-muted: #64748b; --border: #e2e8f0; --radius: 24px; --sidebar-w: 260px; }
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; outline: none; -webkit-tap-highlight-color: transparent; }
        ...
        .nav-btn { display: flex; align-items: center; gap: 12px; padding: 14px 16px; border-radius: 12px; border: none; background: transparent; color: var(--text-muted); font-weight: 600; font-size: 0.95rem; cursor: pointer; transition: 0.2s; width: 100%; text-align: left; }
        ...
        .user-profile { display: flex; align-items: center; gap: 10px; padding: 12px; background: var(--bg-body); border-radius: 16px; margin-top: auto; cursor: pointer; border: 1px solid var(--border); }
        ...
        .podium-slot { display: flex; align-items: center; gap: 15px; padding: 15px; border-radius: 16px; background: var(--bg-body); border: 1px solid var(--border); }
    `}</style>

    {view === 'login' ? (
        <div id="gate" style={{width:'100vw', height:'100vh', display:'flex', justifyContent:'center', alignItems:'center', background:'var(--bg-body)'}}>
            ...
            <main className="main-content">
                ...
                {currentTab === 'home' && (
                    ...
                    <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:20, marginBottom:40}}>
                        {topVendeurs.map((e,i)=>(
                            <div key={e.nom} className="podium-slot">
                    ...
                    <div className="dashboard-grid">
                        <div className="dash-card" onClick={()=>setCurrentTab('invoices')}><Icon name="receipt" size={28} /><strong>Caisse</strong><p>Ventes clients</p></div>
                        <div className="dash-card" onClick={()=>setCurrentTab('stock')}><Icon name="package" size={28} /><strong>Stock</strong><p>Productions cuisine</p></div>
                    </div>
                    </>
                )}
                ...
                {currentTab === 'team' && (
                    <div className="dashboard-grid">
                        {data.employees.map(emp => (
                            <div key={emp.nom} className="dash-card" style={{flexDirection:'row', alignItems:'center', gap:15, height:'auto'}}>
                                <div className="avatar">{emp.nom.charAt(0)}</div>
                                <div>
                                    <strong>{emp.nom}</strong><br/><small>{emp.poste}</small><br/>
                                    <span style={{color:'var(--primary)', fontWeight:800}}>📞 {emp.tel}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                {/* FORMULAIRES AVEC ANTI-DOUBLON */}
                {currentTab === 'stock' && (
                    <div className="form-wrap">
                        <h2>📦 Déclarer Production</h2>
                        ...
                        <button className="btn-primary" disabled={submitting} onClick={()=>sendForm('sendProduction', {items:stockItems})}>{submitting ? 'Envoi...' : 'Envoyer Stock'}</button>
                    </div>
                )}
            </main>

            <aside className={`cart-drawer ${cartOpen ? 'open' : ''}`}>
                <div style={{padding:25, borderBottom:'1px solid var(--border)'}}><h2>Panier</h2></div>
                ...
                <div style={{padding:25, background:'var(--bg-body)'}}>
                    <button className="btn-primary" disabled={submitting || !invNum.trim()} onClick={handleSendInvoice}> {submitting ? 'Validation...' : 'Valider la Vente'} </button>
                </div>
            </aside>
