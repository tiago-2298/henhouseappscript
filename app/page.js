...
export default function Home() {
  ...
  const [sending, setSending] = useState(false); // ANTI-SPAM GLOBAL
  ...
  const loadData = async (isSync = false) => {
    if(isSync) notify("CLOUD", "Synchronisation en cours...");
    try {
  ...
  const send = async (action, payload) => {
    if(sending) return; // BLOQUE L'ENVOI SI DÉJÀ EN COURS (ANTISPAM)
    setSending(true);
    playSound('click');
    ...
    try {
      ...
      const j = await r.json();
      if(j.success) { 
        const successMsg = action === 'sendFactures' ? "Facture Transmise !" : "Données transmises !";
        notify("SUCCÈS", successMsg, "success"); 
        if(action === 'sendFactures') setCart([]); 
        loadData(); 
      } else notify("ERREUR", j.message || "Échec de l'envoi", "error");
    } catch(e) {
      notify("ERREUR", "Connexion perdue", "error");
    } finally { 
      setSending(false); 
    }
  };
    ...
    <div className="app">
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin:0; padding:0; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .nav-l { display: flex; align-items: center; gap: 10px; padding: 12px 16px; border-radius: 12px; border:none; background:transparent; color: var(--muted); cursor: pointer; font-weight: 700; width: 100%; transition: 0.2s; font-size: 0.85rem; margin-bottom: 2px; }
        ...
        .btn-p:disabled { opacity: 0.5; cursor: not-allowed; filter: grayscale(1); }

        /* NOTIFICATIONS PRO */
        .toast { position: fixed; top: 30px; right: 30px; padding: 20px 35px; border-radius: 16px; color: #fff; z-index: 2000; font-weight: 900; box-shadow: 0 15px 40px rgba(0,0,0,0.6); display: flex; flex-direction: column; gap: 4px; animation: toastIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); border-left: 6px solid #fff; }
        @keyframes toastIn { from { transform: translateX(120%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
        .toast-t { font-size: 0.7rem; opacity: 0.8; letter-spacing: 2px; }
        .toast-m { font-size: 1.1rem; }
      ...
      {view === 'login' ? (
        <div style={{flex:1, display:'flex', alignItems:'center', justifyContent:'center'}}>
          <div className="form-ui" style={{textAlign: 'center', maxWidth: 400}}>
            ...
            <h1 style={{marginBottom:35}}>HEN HOUSE</h1>
            <select className="inp" value={user} onChange={e=>setUser(e.target.value)}>
              <option value="">👤 Votre identité...</option>
        ...
        <>
          <aside className="side">
            ...
            {/* BLOC SYNC & REFRESH AU DESSUS DE DÉCO */}
                  ...
                  <div className="form-ui">
                    {currentTab === 'stock' && (
                      <>
                        ...
                        <button className="nav-l" style={{border:'1px dashed var(--brd)', justifyContent:'center', margin:'10px 0 20px'}} onClick={()=>setForms({...forms, stock:[...forms.stock, {product:'', qty:1}]})}>+ Ajouter</button>
                        <button className="btn-p" disabled={sending} onClick={()=>send('sendProduction', {items: forms.stock})}>VALIDER STOCK</button>
                      </>
                    )}
                    ...
                    {currentTab === 'enterprise' && (
                      <>
                        ...
                        <button className="btn-p" disabled={sending} onClick={()=>send('sendEntreprise', forms.enterprise)}>ENVOYER COMMANDE</button>
                      </>
                    )}
                    ...
                    {currentTab === 'partners' && (
                      <>
                        ...
                        <h4 style={{margin: '10px 0'}}>Menu</h4>
                        ...
                        <button className="btn-p" disabled={sending} onClick={()=>send('sendPartnerOrder', forms.partner)}>VALIDER COMMANDE</button>
                      </>
                    )}
                    ...
                    {currentTab === 'garage' && (
                      <>
                        ...
                        <button className="btn-p" disabled={sending} style={{marginTop:25}} onClick={()=>send('sendGarage', forms.garage)}>ENREGISTRER</button>
                      </>
                    )}
                  </div>
                </div>
                  ...
                  <div className="card" style={{padding:30, textAlign:'left'}}>
                    ...
                    {data.employeesFull.sort((a,b)=>b.ca-a.ca).slice(0,10).map((e,i)=>(
                      <div key={i} style={{marginBottom: 15}}>
                        ...
                        <div style={{height: 8, background:'rgba(255,255,255,0.05)', borderRadius:10, marginTop:8}}>
                          <div style={{height:'100%', background:'var(--p)', width:(e.ca/data.employeesFull[0].ca*100)+'%', borderRadius:10}}></div>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="card" style={{padding:30, textAlign:'left'}}>
                    <h2 style={{marginBottom:20}}>📦 TOP 10 STOCK</h2>
                    {data.employeesFull.sort((a,b)=>b.stock-a.stock).slice(0,10).map((e,i)=>(
                      <div key={i} style={{marginBottom: 15}}>
                        ...
                        <div style={{height: 8, background:'rgba(255,255,255,0.05)', borderRadius:10, marginTop:8}}>
                          <div style={{height:'100%', background:'#10b981', width:(e.stock/data.employeesFull[0].stock*100)+'%', borderRadius:10}}></div>
                        </div>
                      </div>
                    ))}
                  </div>
                   ...
                   <div className="form-ui" style={{maxWidth: 600, padding: 50}}>
                      ...
                      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:20}}>
                         <div className="card" style={{background: 'rgba(0,0,0,0.3)'}}>
                            <p style={{fontSize:'0.8rem', color:'var(--muted)'}}>💰 CA TOTAL</p>
                            <p style={{fontSize: '1.5rem', fontWeight: 900}}>${myProfile.ca.toLocaleString()}</p>
                         </div>
                         <div className="card" style={{background: 'rgba(0,0,0,0.3)'}}>
                            <p style={{fontSize:'0.8rem', color:'var(--muted)'}}>📦 STOCK PROD.</p>
                            <p style={{fontSize: '1.5rem', fontWeight: 900}}>{myProfile.stock.toLocaleString()} u.</p>
                         </div>
                      </div>
                      <div className="card" style={{textAlign: 'left', background: 'rgba(255,255,255,0.02)', marginTop: 20}}>
                        <p style={{marginBottom: 10}}>📅 <b>Ancienneté :</b> {myProfile.seniority} jours</p>
                        <p>🆔 <b>ID Employé :</b> #00{myProfile.id}</p>
                      </div>
                   </div>
                </div>
              )}
      ...
      {toast && (
        <div className="toast" style={{background: toast.s === 'error' ? '#dc2626' : '#16a34a'}}>
          <div className="toast-t">{toast.t}</div>
