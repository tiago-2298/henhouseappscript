        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="form-ui fade-in" style={{ textAlign: 'center', maxWidth: 450, zIndex: 10, padding: '50px 40px', background: 'rgba(10, 10, 10, 0.85)', border: '1px solid rgba(255,152,0,0.2)', boxShadow: '0 0 50px rgba(0,0,0,0.8), inset 0 0 20px rgba(255,152,0,0.05)' }}>
            ...
            <button 
              ...
              disabled={!user}
              style={{ height: 60, fontSize: '1.1rem', letterSpacing: '2px', background: user ? 'linear-gradient(90deg, #ff9800, #f59e0b)' : '#222', color: user ? '#000' : '#555', border: 'none', borderRadius: '16px', cursor: user ? 'pointer' : 'not-allowed', boxShadow: user ? '0 10px 25px var(--p-glow)' : 'none' }}
            >
           ...
           {/* ========================================== */}
              ...
              {currentTab === 'enterprise' && (() => {
                  ...
                  const handleAddProduct = (productName) => {
                      ...
                      if (existing) {
                          const newItems = itemsList.map(i => i.product === productName ? { ...i, qty: Number(i.qty) + 1 } : i);
                          setEntForm({ ...entForm, items: newItems });
                      } else {
                          setEntForm({ ...entForm, items: [...itemsList, { product: productName, qty: 1 }] });
                      }
                      playSound('click');
                  };
                      ...
                      <div className="fade-in" style={{ display: 'flex', gap: '30px', height: 'calc(100vh - 120px)', maxHeight: 'calc(100vh - 120px)', maxWidth: '1400px', margin: '0 auto', overflow: 'hidden' }}>
                          ...
                          <div style={{ flex: '0 0 450px', background: 'rgba(15, 15, 15, 0.7)', backdropFilter: 'blur(20px)', borderRadius: '40px', border: '1px solid rgba(255,255,255,0.05)', boxShadow: '0 30px 80px rgba(0,0,0,0.8)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                              <div style={{ padding: '30px', background: 'rgba(0,0,0,0.3)', borderBottom: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
                                  ...
                                  <input type="text" className="inp" placeholder="Nom de l'entreprise (ex: Benny's)" value={entForm.company || entForm.name} onChange={e => setEntForm({ ...entForm, company: e.target.value })} style={{ width: '100%', height: '55px', fontSize: '1rem', fontWeight: 800, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', borderRadius: '16px', padding: '0 20px', marginBottom: 0 }} />
                              </div>
                              ...
                              <div style={{ padding: '30px', background: 'rgba(0,0,0,0.5)', borderTop: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
                                  ...
                                  <button disabled={sending || itemsList.length === 0 || (!entForm.company && !entForm.name)} onClick={() => send('sendEntreprise', entForm)} style={{ width: '100%', padding: '22px', borderRadius: '24px', border: 'none', background: (sending || itemsList.length === 0 || (!entForm.company && !entForm.name)) ? '#222' : 'linear-gradient(90deg, #f59e0b, #ea580c)', color: (sending || itemsList.length === 0 || (!entForm.company && !entForm.name)) ? '#555' : '#fff', fontSize: '1.1rem', fontWeight: 900, cursor: (sending || itemsList.length === 0 || (!entForm.company && !entForm.name)) ? 'not-allowed' : 'pointer', boxShadow: (sending || itemsList.length === 0 || (!entForm.company && !entForm.name)) ? 'none' : '0 10px 30px rgba(245,158,11,0.4)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px', transition: 'all 0.3s' }}>
