...
export default function Home() {
  ...
  const playSound = (type) => {
    ...
    try {
      ...
      if (type === 'click') {
        ...
        gain.gain.exponentialRampToValueAtTime(0.01, now + 0.05); osc.start(); osc.stop(now + 0.05);
      } else if (type === 'success') {
        osc.frequency.setValueAtTime(523, now); osc.frequency.setValueAtTime(783, now + 0.1);
  ...
  const loadData = async (isSync = false) => {
    ...
    } catch (e) { notify("ERREUR CLOUD", "Serveur injoignable", "error"); }
  ...
  const salaryGain = useMemo(() => Math.floor(total * 0.45), [total]); // Centimes supprimés
  const myProfile = useMemo(() => data?.employeesFull?.find(e => e.name === user), [data, user]);

  const updateCartQty = (idx, val) => {
    const n = [...cart];
    const v = parseInt(val) || 0;
    if (v <= 0) n.splice(idx, 1); else n[idx].qty = v;
    setCart(n);
  };
    ...
    <div className="app">
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin:0; padding:0; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .tool-bar { display: flex; gap: 8px; margin-bottom: 15px; }
        .icon-tool { background: rgba(255,255,255,0.05); border: 1px solid var(--brd); color: #fff; flex: 1; padding: 10px; border-radius: 10px; cursor: pointer; font-size: 1.1rem; transition: 0.2s; }
        .icon-tool:hover { background: var(--p); border-color: var(--p); transform: translateY(-2px); }
        ...
        <>
          <aside className="side">
            ...
            <div style={{padding: '20px 0', borderTop: '1px solid var(--brd)'}}>
              <div className="tool-bar">
                <button className="icon-tool" title="Actualiser" onClick={() => window.location.reload()}>🔃</button>
                <button className="icon-tool" title="Synchro Cloud" onClick={() => loadData(true)}>☁️</button>
                <button className="icon-tool" title="Sons" onClick={() => {setIsMuted(!isMuted); playSound('click');}}>
                  {isMuted ? '🔇' : '🔊'}
                </button>
              </div>

              <div style={{background: 'rgba(255,255,255,0.03)', padding: 14, borderRadius: 16, marginBottom: 12, border:'1px solid var(--brd)'}}>
                <div style={{fontSize: '0.9rem', fontWeight: 900, color: '#fff'}}>{user}</div>
                <div style={{fontSize: '0.7rem', color: 'var(--p)', fontWeight: 800, textTransform: 'uppercase', marginTop:2}}>{myProfile?.role || 'Agent'}</div>
              </div>
                ...
                <div className="fade-in">
                   <div style={{marginBottom:45}}>
                       ...
                       <div style={{display:'flex', alignItems:'center', gap:10}}>
                           <span style={{padding:'4px 12px', background:'var(--p)', borderRadius:20, fontSize:'0.7rem', fontWeight:900, color:'#fff'}}>CONNECTÉ</span>
                           <p style={{color: 'var(--muted)', fontSize: '1.05rem'}}>Gérez vos ventes et votre production Hen House.</p>
                       </div>
                   </div>
                   ...
                   <div style={{display:'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20, marginBottom: 45}}>
                      ...
                      <div className="card" style={{display:'flex', alignItems:'center', gap:25, padding: 35, textAlign:'left', background: 'linear-gradient(135deg, #181a20 0%, #0d2e21 100%)', border:'1px solid rgba(16,185,129,0.2)'}}>
                         <div style={{fontSize: '3.5rem'}}>📦</div>
                         <div>
                            <div style={{fontSize: '0.75rem', color:'var(--muted)', fontWeight: 800, letterSpacing: 1, marginBottom:5}}>PRODUCTION</div>
                   ...
                   <div style={{background:'rgba(255,255,255,0.02)', padding:30, borderRadius:24, border:'1px solid var(--brd)'}}>
                        <h3 style={{marginBottom: 25, fontWeight: 900, color: '#fff', fontSize: '1.1rem'}}>RACCOURCIS</h3>
                ...
                <div className="fade-in">
                  <div style={{display:'flex', flexWrap:'wrap', gap:15, marginBottom:30, alignItems:'center'}}>
                    <div style={{position:'relative', flex:1, minWidth:250}}>
                        ...
                        <input className="inp" placeholder="Rechercher un plat..." style={{marginBottom:0, paddingLeft:45}} onChange={e=>setSearch(e.target.value)} />
                    </div>
                  </div>
                      ...
                      return (
                        <div key={p} className={`card ${cartItem?'sel':''}`} onClick={()=>{
                            ...
                            <div style={{height:120, borderRadius:15, overflow:'hidden', background:'#000', marginBottom:12}}>
                                {IMAGES[p] ? <img src={IMAGES[p]} style={{width:'100%', height:'100%', objectFit:'cover'}} /> : <div style={{height:'100%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'2rem'}}>{p.charAt(0)}</div>}
                            </div>
                  ...
                  <div className="form-ui">
                    {currentTab === 'expenses' && (
                      <div className="fade-in">
                        ...
                        <p style={{textAlign:'center', color:'var(--muted)', fontSize:'0.85rem', marginBottom:30}}>Entrez des montants ronds uniquement ($)</p>
                        ...
                        {/* FIX: Pas de centimes, montant forcé en entier */}
                        <input className="inp" type="number" step="1" placeholder="Montant ($)" value={forms.expense.amount} onChange={e=>setForms({...forms, expense:{...forms.expense, amount: Math.floor(e.target.value)}})} />
                        <div className={`dropzone ${dragActive ? 'active' : ''}`} onDragOver={e => { e.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)} onDrop={e => { e.preventDefault(); setDragActive(false); handleFileChange(e.dataTransfer.files[0]); }} onClick={() => document.getElementById('inpFile').click()}>
                           ...
                           {forms.expense.file ? (
                             <div><div style={{color:'var(--p)', fontWeight:900, fontSize:'0.75rem', marginBottom:10}}>PHOTO OK ✅</div><img src={forms.expense.file} className="dz-preview" style={{maxHeight:150, borderRadius:8}} /></div>
                           ) : (
                             <div><div style={{fontSize:'2.5rem', marginBottom:10}}>📸</div><div style={{fontWeight:900, fontSize:'0.9rem'}}>PREUVE OBLIGATOIRE</div></div>
                           )}
                        </div>
                      ...
                      <div className="fade-in">
                        ...
                        {forms.stock.map((item, i) => (
                          <div key={i} style={{display:'flex', gap:12, marginBottom:12}}>
                            <select className="inp" style={{flex:1, marginBottom:0}} value={item.product} onChange={e=>{ const n=[...forms.stock]; n[i].product=e.target.value; setForms({...forms, stock:n}); }}><option value="">Produit...</option>{data.products.map(p=><option key={p} value={p}>{p}</option>)}</select>
                        ...
                        <button className="nav-l" style={{border:'2px dashed var(--brd)', justifyContent:'center', margin:'10px 0 25px'}} onClick={()=>setForms({...forms, stock:[...forms.stock, {product:'', qty:1}]})}>+ LIGNE</button>
                    ...
                    {currentTab === 'enterprise' && (
                      <div className="fade-in">
                        ...
                        <button className="btn-p" style={{marginTop:20}} disabled={sending || !forms.enterprise.name || forms.enterprise.items.some(s => !s.product)} onClick={()=>send('sendEntreprise', {company: forms.enterprise.name, items: forms.enterprise.items})}>ENVOYER</button>
                      </div>
                    )}

                    {currentTab === 'partners' && (
                      <div className="fade-in">
                        <h2 style={{marginBottom:30, textAlign:'center', fontWeight:900}}>🤝 CONTRATS PARTENAIRES</h2>
                        ...
                        {forms.partner.items.map((item, idx) => (
                           <div key={idx} style={{display:'flex', gap:10, marginBottom:10}}>
                             <select className="inp" style={{flex:1}} value={item.menu} onChange={e=>{ const n = [...forms.partner.items]; n[idx].menu = e.target.value; setForms({...forms, partner:{...forms.partner, items:n}}); }}>
                        ...
                        <button className="btn-p" style={{marginTop:15}} disabled={sending || !forms.partner.num} onClick={()=>send('sendPartnerOrder', forms.partner)}>VALIDER PARTENAIRE</button>
                      </div>
                    )}

                    {currentTab === 'garage' && (
                      <div className="fade-in">
                        <h2 style={{marginBottom:30, textAlign:'center', fontWeight:900}}>🚗 GARAGE</h2>
                        ...
                        <div style={{background:'rgba(0,0,0,0.2)', padding:25, borderRadius:20, marginTop:10}}>
                      ...
                      <div className="fade-in center-box" style={{width:'100%'}}>
                        <div className="form-ui" style={{maxWidth: 600, padding: 50}}>
                          <div style={{textAlign:'center', marginBottom: 40}}>
                            <div style={{width:140, height:140, borderRadius:45, background:'var(--p)', margin:'0 auto 25px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'4.5rem', fontWeight:950, color:'#fff'}}>{user.charAt(0)}</div>
                            <h1 style={{fontSize:'2.6rem', fontWeight:950}}>{user}</h1>
                            <div style={{color:'var(--p)', fontWeight:900, fontSize:'1.1rem'}}>{myProfile.role}</div>
                          </div>
                          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom: 20}}>
                              <div className="card" style={{background: 'rgba(255,255,255,0.03)'}}>
                                <p style={{fontSize:'0.75rem', color:'var(--muted)'}}>C.A PERSONNEL</p>
                                <p style={{fontSize: '1.6rem', fontWeight: 950}}>${myProfile.ca.toLocaleString()}</p>
                              </div>
                              <div className="card" style={{background: 'rgba(255,255,255,0.03)'}}>
                                <p style={{fontSize:'0.75rem', color:'var(--muted)'}}>STOCK PRODUIT</p>
                                <p style={{fontSize: '1.6rem', fontWeight: 950}}>{myProfile.stock.toLocaleString()}</p>
                              </div>
                          </div>
                          <div className="card" style={{background: 'linear-gradient(135deg, rgba(255,152,0,0.15) 0%, rgba(24,26,32,1) 100%)', border: '1px solid var(--p)', padding:25}}>
                              <p style={{fontSize:'0.8rem', color:'var(--p)', fontWeight: 900}}>💶 SALAIRE ESTIMÉ</p>
                    ...
                    {currentTab === 'support' && (
                      <div className="fade-in">
                        <h2 style={{marginBottom:30, textAlign:'center', fontWeight:900}}>🆘 SUPPORT</h2>
              ...
              {currentTab === 'performance' && (
                <div className="fade-in" style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(400px, 1fr))', gap:30}}>
                  <div className="card" style={{padding:35, textAlign:'left'}}>
                    <h2 style={{marginBottom:30}}>🏆 TOP C.A</h2>
                    {data.employeesFull.sort((a,b)=>b.ca-a.ca).slice(0,10).map((e,i)=>(
                      <div key={i} style={{marginBottom: 20}}>
                        <div style={{display:'flex', justifyContent:'space-between', fontSize: '1rem', marginBottom:8}}><span>{i+1}. <b>{e.name}</b></span><b>${e.ca.toLocaleString()}</b></div>
                        <div className="perf-bar"><div className="perf-fill" style={{width: (e.ca / Math.max(...data.employeesFull.map(x=>x.ca)) * 100) + '%'}}></div></div>
                      </div>
                    ))}
                  </div>
                  <div className="card" style={{padding:35, textAlign:'left'}}>
                    <h2 style={{marginBottom:30}}>📦 TOP PRODUCTION</h2>
                    {data.employeesFull.sort((a,b)=>b.stock-a.stock).slice(0,10).map((e,i)=>(
                      <div key={i} style={{marginBottom: 20}}>
                        <div style={{display:'flex', justifyContent:'space-between', fontSize: '1rem', marginBottom:8}}><span>{i+1}. <b>{e.name}</b></span><b>{e.stock.toLocaleString()} u.</b></div>
              ...
              {currentTab === 'directory' && (
                <div className="fade-in grid">
                    {data.employeesFull.map(e => (
                        ...
                        <div style={{width:60, height:60, borderRadius:20, background:'var(--panel)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.5rem', fontWeight:950}}>{e.name.charAt(0)}</div>
                        <div style={{flex:1}}>
                            <div style={{fontWeight:900}}>{e.name}</div>
                            <div style={{fontSize:'0.7rem', color:'var(--p)', fontWeight:800}}>{e.role}</div>
                            <div style={{fontSize:'0.9rem', marginTop:5, color:'var(--muted)'}}>📞 {e.phone}</div>
                        </div>
                        </div>
                    ))}
                </div>
          ...
          {currentTab === 'invoices' && (
            <aside className="cart">
              ...
              <div style={{padding:20}}><input className="inp" placeholder="N° FACTURE" value={forms.invoiceNum} onChange={e=>setForms({...forms, invoiceNum:e.target.value})} style={{textAlign:'center', fontSize:'1.3rem'}} /></div>
              <div style={{flex:1, overflowY:'auto', padding:'10px 20px'}}>
                {cart.length === 0 ? (
                    <div style={{height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', opacity:0.2}}><span>🛍️</span><p>VIDE</p></div>
                ) : cart.map((i, idx)=>(
                  <div key={idx} style={{display:'flex', justifyContent:'space-between', padding:'15px 0', borderBottom:'1px solid rgba(255,255,255,0.05)', alignItems:'center'}}>
                    <div style={{flex:1}}><div style={{fontWeight:800, fontSize:'0.85rem'}}>{i.name}</div><div style={{color:'var(--p)', fontSize:'0.8rem'}}>${i.pu}</div></div>
                    <div style={{display:'flex', alignItems:'center', gap:10}}>
                      <button style={{background:'var(--brd)', border:'none', color:'#fff', width:30, height:30, borderRadius:8}} onClick={()=>{const n=[...cart]; if(n[idx].qty>1) n[idx].qty--; else n.splice(idx,1); setCart(n);}}>-</button>
                      <input className="qty-inp" type="number" value={i.qty} onChange={(e) => updateCartQty(idx, e.target.value)} />
                      <button style={{background:'var(--brd)', border:'none', color:'#fff', width:30, height:30, borderRadius:8}} onClick={()=>{const n=[...cart]; n[idx].qty++; setCart(n);}}>+</button>
                    </div>
                  </div>
                ))}
              </div>
              <div style={{padding:30, background:'#0a0a0a', borderTop:'1px solid var(--brd)'}}>
                {total > 0 && <div style={{textAlign:'center'}}><div className="salary-badge"><span>💸</span> Gain salaire : <b>+ ${salaryGain}</b></div></div>}
                ...
                <button className="btn-p" style={{height:60}} disabled={sending || !forms.invoiceNum || cart.length === 0} onClick={()=>send('sendFactures', {invoiceNumber: forms.invoiceNum, items: cart.map(x=>({desc:x.name, qty:x.qty}))})}>VALIDER LA VENTE</button>
              </div>
            </aside>
          )}
        </>
