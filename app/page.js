...
const NOTIF_MESSAGES = {
  ...
  sendProduction: { title: "📦 STOCK ACTUALISÉ", msg: "Production validée." },
  sendEntreprise: { title: "🏢 COMMANDE PRO", msg: "Contrat envoyé." },
  sendPartnerOrder: { title: "🤝 PARTENAIRE VALIDÉ", msg: "Commande partenaire reçue." },
  sendGarage: { title: "🚗 GARAGE", msg: "État du véhicule mis à jour." },
  sendExpense: { title: "💳 FRAIS", msg: "Note de frais transmise." },
  sendSupport: { title: "🆘 SUPPORT", msg: "Ticket envoyé au patron." },
  sync: { title: "☁️ SYNC CLOUD", msg: "Données à jour." }
};
...
export default function Home() {
  ...
  const loadData = async (isSync = false) => {
    if(isSync) notify("SYNC", "Connexion Cloud...", "info");
    try {
    ...
    } catch (e) { notify("ERREUR", "Cloud inaccessible", "error"); }
  ...
  const send = async (action, payload) => {
    ...
    try {
      ...
      const j = await r.json();
      if(j.success) { 
        const m = NOTIF_MESSAGES[action] || { title: "SUCCÈS", msg: "Données transmises" };
    ...
    } catch (e) { notify("ERREUR", "Serveur muet", "error"); }
    ...
    <div className="app">
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin:0; padding:0; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        ::-webkit-scrollbar-thumb { background: #2d333f; border-radius: 10px; }
        ::-webkit-scrollbar-thumb:hover { background: var(--p); }

        .app { display: flex; height: 100vh; width: 100vw; }
        .side { width: 250px; border-right: 1px solid var(--brd); padding: 20px; display: flex; flex-direction: column; background: #000; flex-shrink: 0; }
        ...
        .main { flex: 1; overflow-y: auto; padding: 25px; background: radial-gradient(circle at 100% 100%, #2a1b0a 0%, #0b0d11 100%); position: relative; }
        ...
        .form-ui { width: 100%; max-width: 550px; background: rgba(22, 25, 32, 0.6); backdrop-filter: blur(12px); padding: 35px; border-radius: 30px; border: 1px solid var(--brd); box-shadow: 0 20px 50px rgba(0,0,0,0.5); }
        
        .inp { width: 100%; padding: 12px; border-radius: 10px; border: 1px solid var(--brd); background: #0b0d11; color: #fff; font-weight: 600; margin-bottom: 12px; }
        .btn-p { background: var(--p); color: #fff; border:none; padding: 15px; border-radius: 12px; font-weight: 800; cursor: pointer; width: 100%; transition: 0.2s; }
        .btn-p:disabled { background: #374151; color: #9ca3af; cursor: not-allowed; opacity: 0.6; }

        /* CART REDUCED */
        .cart { width: 320px; border-left: 1px solid var(--brd); background: var(--panel); display: flex; flex-direction: column; flex-shrink: 0; }
        .qty-inp { width: 50px; background: #000; border: 1px solid var(--brd); color: #fff; text-align: center; border-radius: 6px; font-weight: 800; padding: 4px 0; }
        ...
        <>
          <aside className="side">
            ...
            <div style={{padding: '15px 0', borderTop: '1px solid var(--brd)'}}>
              <div className="tool-bar">
                <button className="icon-tool" title="Refresh" onClick={() => window.location.reload()}>🔃</button>
                <button className="icon-tool" title="Sync" onClick={() => loadData(true)}>☁️</button>
                <button className="icon-tool" onClick={() => {setIsMuted(!isMuted); playSound('click');}}>{isMuted ? '🔇' : '🔊'}</button>
              </div>
            ...
            <div className="fade-in">
              {currentTab === 'home' && (
                <div style={{animation: 'slideIn 0.5s ease'}}>
                   <h1 style={{fontSize: '2.2rem', fontWeight: 900, marginBottom: 10}}>Bonjour, {user.split(' ')[0]} 👋</h1>
                   <p style={{color: 'var(--muted)', fontSize: '1rem', marginBottom: 40}}>Bienvenue sur le portail Hen House. Gérez vos ventes et productions.</p>
                   <div style={{display:'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20, marginBottom: 40}}>
                      <div className="card" style={{display:'flex', alignItems:'center', gap:25, padding: 30, textAlign:'left', background: 'linear-gradient(135deg, var(--panel) 0%, #2a1b0a 100%)'}}>
                         <div style={{fontSize: '3rem'}}>💰</div>
                         <div>
                            <div style={{fontSize: '0.7rem', color:'var(--muted)', fontWeight: 800}}>MON CA TOTAL</div>
                            <div style={{fontSize: '1.8rem', fontWeight: 900, color: 'var(--p)'}}>${myProfile?.ca.toLocaleString()}</div>
                         </div>
                      </div>
                      <div className="card" style={{display:'flex', alignItems:'center', gap:25, padding: 30, textAlign:'left', background: 'linear-gradient(135deg, var(--panel) 0%, #1a1a1a 100%)'}}>
                         <div style={{fontSize: '3rem'}}>📦</div>
                         <div>
                            <div style={{fontSize: '0.7rem', color:'var(--muted)', fontWeight: 800}}>MA PRODUCTION</div>
                            <div style={{fontSize: '1.8rem', fontWeight: 900}}>{myProfile?.stock.toLocaleString()} u.</div>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="grid">
                      {MODULES.filter(m => !['home', 'profile'].includes(m.id)).map(m => (
                        <div key={m.id} className="card" onClick={()=>setCurrentTab(m.id)} style={{padding: 20}}>
                           <span style={{fontSize:'2.5rem'}}>{m.e}</span>
                           <div style={{marginTop:10, fontSize:'0.85rem', fontWeight:800}}>{m.l}</div>
                        </div>
                      ))}
                   </div>
                </div>
                  ...
                  <div className="grid">
                    {data.products.filter(p => (catFilter==='Tous' || data.productsByCategory[catFilter]?.includes(p)) && p.toLowerCase().includes(search.toLowerCase())).map(p=>(
                      <div key={p} className={`card ${cart.some(i=>i.name===p)?'sel':''}`} onClick={()=>{
                        ...
                        <div style={{height:100, borderRadius:12, overflow:'hidden', background:'#000', marginBottom:8}}>
                          {IMAGES[p] ? <img src={IMAGES[p]} style={{width:'100%', height:'100%', objectFit:'cover'}} /> : <span style={{lineHeight:'100px', opacity:0.2}}>{p.charAt(0)}</span>}
                        </div>
                        <div style={{fontWeight:800, fontSize:'0.75rem', height:30}}>{p}</div>
                  ...
                  <div className="form-ui">
                    {currentTab === 'profile' && myProfile && (
                      <div style={{textAlign:'center'}}>
                        <div style={{width:100, height:100, borderRadius:35, background:'var(--p)', margin:'0 auto 20px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'3rem', fontWeight:900}}>{user.charAt(0)}</div>
                        <h2 style={{fontSize:'1.8rem', fontWeight:900}}>{user}</h2>
                        <p style={{color:'var(--p)', fontWeight:800, marginBottom: 25}}>{myProfile.role}</p>
                        ...
                        <div className="card" style={{textAlign: 'left', background: 'rgba(255,255,255,0.02)', padding: '15px'}}>
                          <p style={{marginBottom: 8}}>📅 <b>Ancienneté :</b> {myProfile.seniority} jours</p>
                          <p style={{marginBottom: 8}}>🆔 <b>ID Employé :</b> #00{myProfile.id}</p>
                    ...
                    {currentTab === 'stock' && (
                      <>
                        <h2 style={{textAlign:'center', marginBottom:20}}>📦 STOCK CUISINE</h2>
                        ...
                        <button className="nav-l" style={{border:'1px dashed var(--brd)', justifyContent:'center', marginBottom:15}} onClick={()=>setForms({...forms, stock:[...forms.stock, {product:'', qty:1}]})}>+ Ligne</button>
                    ...
                    {currentTab === 'enterprise' && (
                      <>
                        <h2 style={{textAlign:'center', marginBottom:20}}>🏢 COMMANDE PRO</h2>
                    ...
                    {currentTab === 'partners' && (
                      <>
                        <h2 style={{textAlign:'center', marginBottom:20}}>🤝 PARTENAIRES</h2>
                    ...
                    {currentTab === 'support' && (
                      <>
                        <h2 style={{textAlign:'center', marginBottom:20}}>🆘 SUPPORT</h2>
              ...
              {currentTab === 'performance' && (
                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:25}}>
                  <div className="card" style={{padding:20, textAlign:'left'}}>
                    <h2 style={{marginBottom:20}}>🏆 TOP 10 CA</h2>
                    {data.employeesFull.sort((a,b)=>b.ca-a.ca).slice(0,10).map((e,i)=>(
                      <div key={i} style={{marginBottom: 12}}>
                        <div style={{display:'flex', justifyContent:'space-between', fontSize: '0.8rem'}}><span>{i+1}. <b>{e.name}</b></span><b style={{color:'var(--p)'}}>${e.ca.toLocaleString()}</b></div>
                  ...
                  <div className="card" style={{padding:20, textAlign:'left'}}>
                    ...
                    {data.employeesFull.sort((a,b)=>b.stock-a.stock).slice(0,10).map((e,i)=>(
                      <div key={i} style={{marginBottom: 12}}>
                        <div style={{display:'flex', justifyContent:'space-between', fontSize: '0.8rem'}}><span>{i+1}. <b>{e.name}</b></span><b>{e.stock.toLocaleString()} u.</b></div>
                ...
                <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px, 1fr))', gap:15}}>
                  {data.employeesFull.map(e => (
                    <div key={e.id} className="card" style={{padding:15, textAlign:'left', display:'flex', gap:12, alignItems:'center'}}>
                      <div style={{width:45, height:45, borderRadius:12, background:'var(--p)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.1rem', fontWeight:900}}>{e.name.charAt(0)}</div>
                      <div style={{flex:1}}><div style={{fontWeight:800}}>{e.name}</div><div style={{fontSize:'0.65rem', color:'var(--p)', fontWeight:700}}>{e.role}</div><div style={{fontSize:'0.8rem', color:'var(--muted)'}}>📞 {e.phone}</div></div>
                    </div>
                  ))}
                </div>
              )}
          ...
          {currentTab === 'invoices' && (
            <aside className="cart">
              <div style={{padding:20, borderBottom:'1px solid var(--brd)'}}><h2 style={{fontSize:'1rem', fontWeight:900}}>🛒 PANIER</h2></div>
              <div style={{padding:15}}><input className="inp" placeholder="N° FACTURE" value={forms.invoiceNum} onChange={e=>setForms({...forms, invoiceNum:e.target.value})} style={{textAlign:'center', fontSize:'1.1rem'}} /></div>
              <div style={{flex:1, overflowY:'auto', padding:'0 15px'}}>
                {cart.map((i, idx)=>(
                  <div key={idx} style={{display:'flex', justifyContent:'space-between', padding:'10px 0', borderBottom:'1px solid rgba(255,255,255,0.05)', alignItems:'center'}}>
                    ...
                    <div style={{display:'flex', alignItems:'center', gap:5}}>
                      <button style={{background:'var(--brd)', border:'none', color:'#fff', width:24, height:24, borderRadius:6}} onClick={()=>{playSound('click'); const n=[...cart]; if(n[idx].qty>1) n[idx].qty--; else n.splice(idx,1); setCart(n);}}>-</button>
                      <input className="qty-inp" type="number" value={i.qty} onChange={(e) => updateCartQty(idx, e.target.value)} />
                      <button style={{background:'var(--brd)', border:'none', color:'#fff', width:24, height:24, borderRadius:6}} onClick={()=>{playSound('click'); const n=[...cart]; n[idx].qty++; setCart(n);}}>+</button>
                    </div>
                  </div>
                ))}
              </div>
              <div style={{padding:20, background:'rgba(0,0,0,0.5)', borderTop:'1px solid var(--brd)'}}>
                <div style={{display:'flex', justifyContent:'space-between', marginBottom:15}}><span>TOTAL</span><b style={{fontSize:'1.8rem', color:'var(--p)', fontWeight:900}}>${total.toLocaleString()}</b></div>
      ...
      {toast && (
        <div className="toast" style={{
          ...
          <div style={{ fontSize: '0.8rem', fontWeight: 900 }}>{toast.t}</div>
          <div style={{ fontSize: '0.85rem' }}>{toast.m}</div>
        </div>
      )}
    </div>
  );
