...
export default function Home() {
  ...
  const syncData = async () => {
    notify("Synchronisation...", "Mise à jour des données", "info");
    try {
      const res = await fetch('/api', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ action: 'syncData' }) });
      const json = await res.json();
      if (json.success) { setData(json); notify("Succès", "Données synchronisées", "success"); }
  ...
  const addToCart = (prod) => {
    ...
    if(!quickMode) notify("Ajouté", prod, "success");
  };
    ...
    <>
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; outline: none; }
        body { font-family: 'Plus Jakarta Sans', sans-serif; background: var(--bg-dark); color: #f8fafc; height: 100vh; overflow: hidden; display: flex; transition: 0.3s; }
        ...
        .sidebar { width: var(--sidebar-w); height: calc(100vh - 32px); margin: 16px; background: var(--bg-panel); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-radius: var(--radius); display: flex; flex-direction: column; padding: 24px; border: 1px solid var(--border); box-shadow: 0 10px 30px rgba(0,0,0,0.4); z-index: 50; }
        .brand { display: flex; align-items: center; gap: 12px; font-weight: 800; font-size: 1.1rem; margin-bottom: 32px; letter-spacing: -0.02em; }
        .nav-list { display: flex; flex-direction: column; gap: 6px; flex: 1; overflow-y: auto; scrollbar-width: none; }
        .nav-btn { display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-radius: 12px; border: none; background: transparent; color: #94a3b8; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: 0.2s; }
        ...
        .main-content { flex: 1; padding: 32px 32px 32px 16px; overflow-y: auto; position: relative; }
        .header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 32px; }
        .title { font-size: 1.6rem; font-weight: 900; letter-spacing: -0.04em; }

        .dashboard-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 20px; }
        .stat-card { background: var(--bg-panel); padding: 24px; border-radius: var(--radius); border: 1px solid var(--border); transition: 0.3s; cursor: pointer; display: flex; flex-direction: column; justify-content: space-between; height: 160px; }
        .stat-card:hover { transform: translateY(-6px); border-color: var(--primary); box-shadow: 0 20px 40px rgba(0,0,0,0.4); }

        .prod-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(170px, 1fr)); gap: 20px; padding-bottom: 100px; }
        .prod-card { background: var(--bg-panel); border-radius: var(--radius); padding: 12px; border: 1px solid var(--border); transition: 0.2s; text-align: center; cursor: pointer; }
        .prod-card:hover { border-color: var(--primary); transform: scale(1.03); }
        .prod-img { width: 100%; aspect-ratio: 1; border-radius: 14px; object-fit: cover; margin-bottom: 12px; background: #000; }
        ...
        .drawer { position: fixed; top: 16px; right: 16px; bottom: 16px; width: 420px; background: rgba(18,18,22,0.9); backdrop-filter: blur(30px); border-radius: 24px; border: 1px solid var(--border); box-shadow: -20px 0 50px rgba(0,0,0,0.5); z-index: 1000; transform: translateX(110%); transition: 0.4s cubic-bezier(0.4, 0, 0.2, 1); display: flex; flex-direction: column; }
        .drawer.open { transform: translateX(0); }

        .toast { position: fixed; bottom: 32px; right: 32px; z-index: 3000; background: var(--bg-panel); border: 1px solid var(--border); border-left: 5px solid var(--primary); padding: 16px 24px; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); animation: slideUp 0.3s ease; }
        @keyframes slideUp { from { transform: translateY(20px); opacity: 0; } }

        .music-player { margin-top: auto; padding: 15px; background: rgba(255,255,255,0.03); border-radius: 15px; border: 1px solid var(--border); }
        .play-btn { width: 32px; height: 32px; border-radius: 50%; background: var(--primary); border: none; color: white; cursor: pointer; display: flex; align-items: center; justify-content: center; }

        .cat-pills { display: flex; gap: 8px; overflow-x: auto; padding-bottom: 16px; scrollbar-width: none; }
        .pill { padding: 8px 20px; background: rgba(255,255,255,0.03); border: 1px solid var(--border); border-radius: 50px; white-space: nowrap; cursor: pointer; font-weight: 600; font-size: 0.85rem; transition: 0.2s; }
        .pill.active { background: var(--primary); border-color: var(--primary); color: white; }

        .form-container { max-width: 700px; margin: 0 auto; background: var(--bg-panel); padding: 32px; border-radius: 24px; border: 1px solid var(--border); }
        .row-item { display: flex; justify-content: space-between; align-items: center; padding: 12px; border-radius: 12px; transition: 0.2s; }
      ...
      {view === 'login' ? (
        <div id="gate" style={{position:'fixed', inset:0, background:'var(--bg-dark)', display:'flex', alignItems:'center', justify:'center', zIndex:10000}}>
          <div style={{width:400, padding:48, background:'var(--bg-panel)', borderRadius:32, border:'1px solid var(--border)', textAlign:'center', backdropFilter:'blur(20px)'}}>
            <img src="https://i.goopics.net/dskmxi.png" style={{height:70, borderRadius:16, marginBottom:24}} alt="Logo" />
            <h2 style={{fontWeight:900, marginBottom:32, fontSize:'1.8rem', letterSpacing:'-0.05em'}}>Hen House Portal</h2>
        ...
        <>
          <aside className="sidebar">
            <div style={{display:'flex', alignItems:'center', gap:12, marginBottom:32}}>
              ...
              <span style={{fontWeight:900, letterSpacing:'-0.05em'}}>HEN HOUSE</span>
            </div>
            ...
            <div className="music-player">
                <div style={{display:'flex', alignItems:'center', justify:'space-between', marginBottom:10}}>
                    <div><div style={{fontSize:'0.65rem', fontWeight:800}}>MUSIC PLAYER</div><div style={{fontSize:'0.55rem', opacity:0.5}}>{playing ? 'Streaming...' : 'Paused'}</div></div>
            ...
            <div style={{marginTop:'20px', padding:'16px', background:'rgba(255,255,255,0.02)', borderRadius:16, border:'1px solid var(--border)'}}>
              <div style={{display:'flex', gap:10, alignItems:'center', marginBottom:12}}>
                <div style={{width:32, height:32, background:'var(--primary)', borderRadius:8, display:'flex', alignItems:'center', justify:'center', fontWeight:800}}>{user[0]}</div>
                <div style={{fontSize:'0.8rem'}}><b>{user}</b></div>
              </div>
              <button onClick={logout} style={{background:'none', border:'none', color:'#ef4444', fontSize:'0.75rem', cursor:'pointer', fontWeight:700}}>Sign Out</button>
            </div>
          </aside>
          ...
          <main className="main-content">
            <header className="header">
              ...
              <div style={{display:'flex', gap:16, alignItems:'center'}}>
                <div style={{background:'var(--bg-panel)', padding:'10px 20px', borderRadius:50, border:'1px solid var(--border)', fontWeight:800, fontSize:'0.9rem'}}>
            ...
            {currentTab === 'home' && (
              <div className="dashboard-grid">
                <div className="stat-card" onClick={()=>setCurrentTab('invoices')}>
                  <Icon name="receipt" size={32} style={{color:'var(--primary)'}}/>
                  <div><div style={{fontWeight:800}}>Caisse</div><div style={{fontSize:'0.8rem', opacity:0.6}}>Ventes et factures</div></div>
                </div>
                <div className="stat-card" onClick={()=>setCurrentTab('stock')}>
                  <Icon name="package" size={32} style={{color:'var(--primary)'}}/>
                  <div><div style={{fontWeight:800}}>Stock</div><div style={{fontSize:'0.8rem', opacity:0.6}}>Production cuisine</div></div>
                </div>
                <div className="stat-card" onClick={()=>setCurrentTab('performance')}>
                  <Icon name="trophy" size={32} style={{color:'var(--primary)'}}/>
                  <div><div style={{fontWeight:800}}>Performance</div><div style={{fontSize:'0.8rem', opacity:0.6}}>Classements</div></div>
                </div>
              </div>
            )}
              ...
              <>
                <div style={{display:'flex', gap:12, marginBottom:16}}>
                    <div style={{position:'relative', flex:1}}>
                        ...
                        <input className="inp-field" style={{paddingLeft:48}} placeholder="Search 1000+ products..." onChange={e=>setSearch(e.target.value)} />
                    </div>
                </div>
                <div className="cat-pills">
                  <div className={`pill ${catFilter==='Tous'?'active':''}`} onClick={()=>setCatFilter('Tous')}>All</div>
                ...
                <div className="prod-grid">
                  {data?.products?.filter(p => (catFilter==='Tous' || data.productsByCategory[catFilter]?.includes(p)) && p.toLowerCase().includes(search.toLowerCase())).map(p => (
                    <div key={p} className="prod-card" onClick={()=>addToCart(p)}>
                      ...
                      <div style={{fontWeight:700, fontSize:'0.8rem', marginBottom:4}}>{p}</div>
            ...
            {currentTab === 'stock' && (
              <div className="form-container">
                <h2 style={{marginBottom:24, fontWeight:900}}>Déclaration Stock</h2>
                ...
                <button className="btn" style={{width:'100%', border:'1px dashed var(--border)', marginBottom:12}} onClick={()=>setStockItems([...stockItems, {product:data?.products[0], qty:1}])}>+ Add Line</button>
                <button className="btn btn-primary" style={{width:'100%'}} onClick={()=>sendForm('sendProduction', {items:stockItems})}>Envoyer en cuisine</button>
              </div>
            )}

            {currentTab === 'garage' && (
               <div className="form-container">
                 <h2 style={{marginBottom:24, fontWeight:900}}>Garage</h2>
                 ...
                 <button className="btn btn-primary" style={{width:'100%'}} onClick={()=>sendForm('sendGarage', garData)}>Update Vehicle Status</button>
               </div>
            )}
            ...
            {currentTab === 'performance' && (
              <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(320px, 1fr))', gap:24}}>
                <div style={{background:'var(--bg-panel)', padding:32, borderRadius:24, border:'1px solid var(--border)'}}>
                  <h3 style={{marginBottom:20, fontWeight:900}}>Top Revenue ($)</h3>
                  {topCA.map((e, i) => (
                    <div key={e.name} className="row-item" style={{borderBottom:'1px solid var(--border)'}}>
                ...
                <div style={{background:'var(--bg-panel)', padding:32, borderRadius:24, border:'1px solid var(--border)'}}>
                  <h3 style={{marginBottom:20, fontWeight:900}}>Top Production</h3>
                  {topStock.map((e, i) => (
                    <div key={e.name} className="row-item" style={{borderBottom:'1px solid var(--border)'}}>
                      <span>{i+1}. {e.name}</span>
                      <b style={{color:'var(--primary)'}}>{e.stock} u.</b>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <>
              ...
              <aside className={`drawer ${cartOpen ? 'open' : ''}`}>
                <div style={{padding:24, borderBottom:'1px solid var(--border)', display:'flex', justify:'space-between', alignItems:'center'}}>
                  <h2 style={{fontWeight:900}}>Cart</h2>
                  <button onClick={()=>setCartOpen(false)} style={{background:'none', border:'none', color:'white'}}><Icon name="x" size={24}/></button>
                </div>
                <div style={{padding:24}}>
                  <input className="inp-field" style={{textAlign:'center', fontSize:'1.2rem', fontWeight:900}} placeholder="Invoice ID" value={invNum} onChange={e=>setInvNum(e.target.value)} />
                </div>
                <div style={{flex:1, overflowY:'auto', padding:'0 24px'}}>
                  {cart.map((c, i) => (
                    <div key={i} style={{display:'flex', justify:'space-between', padding:'12px 0', borderBottom:'1px solid var(--border)'}}>
                ...
                <div style={{padding:32, background:'rgba(0,0,0,0.2)', borderTop:'1px solid var(--border)'}}>
                  ...
                  <button className="btn btn-primary" style={{width:'100%', height:56}} onClick={()=>{
                    if(!invNum) return notify("Error", "Missing Invoice ID", "error");
          ...
          {toast && <div className="toast"><div style={{fontWeight:900}}>{toast.title}</div><div style={{fontSize:'0.8rem', opacity:0.6}}>{toast.msg}</div></div>}
        </>
      )}
    </>
  );
