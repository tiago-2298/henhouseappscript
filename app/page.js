...
export default function Home() {
  ...
  const send = async (action, payload) => {
    ...
    try {
      ...
      const j = await r.json();
      if(j.success) { 
        notify("Validé", "Action enregistrée", "success"); 
  ...
  return (
    <div className="app">
      <style jsx global>{`
        :root { --p: #8b5cf6; --bg: #0b0d11; --panel: #161920; --txt: #f1f5f9; --muted: #94a3b8; --brd: #2d333f; }
        * { box-sizing: border-box; margin:0; padding:0; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .nav-l { display: flex; align-items: center; gap: 12px; padding: 12px; border-radius: 12px; border:none; background:transparent; color: var(--muted); cursor: pointer; font-weight: 700; width: 100%; transition: 0.2s; font-size: 0.85rem; margin-bottom: 2px; }
        ...
        .sync-tools { position: fixed; bottom: 20px; right: 20px; display: flex; gap: 10px; z-index: 1000; }
        .tool-btn { background: var(--p); color: #fff; width: 45px; height: 45px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: none; cursor: pointer; box-shadow: 0 5px 15px rgba(0,0,0,0.4); font-size: 1.2rem; transition: 0.3s; }
        .tool-btn:hover { transform: scale(1.1) rotate(15deg); filter: brightness(1.2); }
      `}</style>
        ...
        <>
          ...
          <main className="main">
            <div className="sync-tools">
              <button className="tool-btn" title="Forcer rafraîchissement page" onClick={()=>window.location.reload()}>🔃</button>
              <button className="tool-btn" title="Sync Cloud Data" onClick={()=>loadData(true)}>☁️</button>
            </div>
                ...
                <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:20}}>
                   {MODULES.filter(m => m.id !== 'home').map(m => (
                     <div key={m.id} className="card" onClick={()=>setCurrentTab(m.id)}>
                        ...
                        <h3 style={{marginTop:15, fontSize:'0.9rem'}}>{m.l}</h3>
                     </div>
                   ))}
                </div>
              )}
                  ...
                  <div className="form-ui">
                    ...
                    {currentTab === 'garage' && (
                      <>
                        ...
                        <div style={{display:'flex', justifyContent:'space-between', fontWeight:900, marginTop:20}}><span>Niveau Essence</span><span>{forms.garage.fuel}%</span></div>
              ...
              {currentTab === 'profile' && myProfile && (
                <div style={{maxWidth:600, margin:'0 auto', textAlign:'center'}}>
                   ...
                   <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:15}}>
                      ...
                      <div className="card"><span>Stock</span><br/><b>{myProfile.stock}</b></div>
          ...
          {currentTab === 'invoices' && (
            <aside className="cart">
              <div style={{padding:24, borderBottom:'1px solid var(--brd)'}}><h2>🛒 Panier</h2></div>
                ...
                {cart.map((i, idx)=>(
                  <div key={idx} style={{display:'flex', justifyContent:'space-between', padding:'12px 0', borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
                    ...
                    <div style={{display:'flex', alignItems:'center', gap:8}}>
                      <button style={{background:'var(--brd)', border:'none', color:'#fff', width:28, height:28, borderRadius:8}} onClick={()=>{playSound('click'); const n=[...cart]; if(n[idx].qty>1) n[idx].qty--; else n.splice(idx,1); setCart(n);}}>-</button>
                      <b style={{minWidth:25}}>{i.qty}</b>
                      <button style={{background:'var(--brd)', border:'none', color:'#fff', width:28, height:28, borderRadius:8}} onClick={()=>{playSound('click'); const n=[...cart]; n[idx].qty++; setCart(n);}}>+</button>
                    </div>
                  </div>
                ))}
              </div>
              <div style={{padding:25, background:'rgba(0,0,0,0.5)', borderTop:'1px solid var(--brd)'}}>
                <div style={{display:'flex', justifyContent:'space-between', marginBottom:20}}><span>TOTAL</span><b style={{fontSize:'2.2rem', color:'var(--p)'}}>${total.toLocaleString()}</b></div>
      ...
      {toast && <div className="toast" style={{position:'fixed', top:20, right:20, background: toast.s==='error'?'#dc2626':'#8b5cf6', padding:'15px 30px', borderRadius:12, zIndex:2000}}><b>{toast.t}</b> : {toast.m}</div>}
    </div>
  );
}
