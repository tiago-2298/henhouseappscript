...
export default function Home() {
  ...
  const loadData = async (isSync = false) => {
    if(isSync) notify("Sync Cloud", "Mise à jour des données...");
    try {
    ...
    <div className="app">
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin:0; padding:0; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .center-form { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 85%; }
        .form-card { width: 100%; max-width: 580px; background: rgba(22, 25, 32, 0.6); backdrop-filter: blur(15px); padding: 45px; border-radius: 35px; border: 1px solid var(--brd); box-shadow: 0 30px 60px rgba(0,0,0,0.6); }
        ...
        .btn-p { background: var(--p); color: #fff; border:none; padding: 18px; border-radius: 14px; font-weight: 800; cursor: pointer; width: 100%; transition: 0.2s; }
      ...
      {view === 'login' ? (
        <div style={{flex:1, display:'flex', alignItems:'center', justifyContent:'center'}}>
          <div className="form-card" style={{textAlign: 'center', maxWidth: 420}}>
            ...
            <h1 style={{marginBottom:35, fontSize:'1.6rem', fontWeight:900}}>CONNEXION</h1>
        ...
        <>
          <aside className="side">
            ...
            <div style={{padding:15, background:'rgba(255,255,255,0.03)', border:'1px solid var(--brd)', borderRadius:16}}>
               ...
               <div style={{fontSize:'0.7rem', color:'var(--p)', fontWeight:700}}>{myProfile?.role || 'Employé'}</div>
               <button className="nav-l" onClick={()=>{playSound('click'); setView('login');}} style={{color:'#ef4444', padding:0, marginTop:12, height:'auto'}}>🚪 Déconnexion</button>
            </div>
          </aside>

          <main className="main">
            <div className="sync-tools">
              <button className="tool-btn" title="Recharger la page" onClick={()=>window.location.reload()}>🔃</button>
                ...
                <>
                  ...
                  <div className="grid">
                    {data.products.filter(p => (catFilter==='Tous' || data.productsByCategory[catFilter]?.includes(p)) && p.toLowerCase().includes(search.toLowerCase())).map(p=>(
                      <div key={p} className="card" onClick={()=>{playSound('click'); addToCart(p);}}>
                  ...
                  <div className="form-card">
                    {currentTab === 'stock' && (
                      <>
                        ...
                        <button className="nav-l" style={{border:'1px dashed var(--brd)', justifyContent:'center', margin:'10px 0 25px'}} onClick={()=>setForms({...forms, stock:[...forms.stock, {product:'', qty:1}]})}>+ Ajouter une ligne</button>
                    ...
                    {currentTab === 'garage' && (
                      <>
                        ...
                        <div style={{display:'flex', justifyContent:'space-between', fontWeight:900, marginTop:25}}><span>⛽ CARBURANT</span><span>{forms.garage.fuel}%</span></div>
                    ...
                    {currentTab === 'support' && (
                      <>
                        <h2 style={{marginBottom:30, textAlign:'center'}}>🆘 SUPPORT</h2>
              ...
              {currentTab === 'profile' && myProfile && (
                <div style={{maxWidth:700, margin:'50px auto'}}>
                   <div style={{display:'flex', gap:40, alignItems:'center', marginBottom:50}}>
                      <div style={{width:130, height:130, borderRadius:40, background:'var(--p)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'4rem', fontWeight:900}}>{user.charAt(0)}</div>
                      <div><h1>{user}</h1><p style={{color:'var(--p)', fontSize:'1.2rem', fontWeight:800}}>{myProfile.role}</p></div>
                   </div>
                   <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:25}}>
                      <div className="card" style={{padding:30, textAlign:'left'}}>
                        <span style={{fontSize:'0.8rem', color:'var(--muted)'}}>CA TOTAL</span><b style={{fontSize:'1.8rem', display:'block'}}>${myProfile.ca.toLocaleString()}</b>
                      </div>
                      <div className="card" style={{padding:30, textAlign:'left'}}>
                        <span style={{fontSize:'0.8rem', color:'var(--muted)'}}>UNITÉS PRODUITES</span><b style={{fontSize:'1.8rem', display:'block'}}>{myProfile.stock}</b>
                      </div>
                   </div>
                </div>
              )}
          ...
          {currentTab === 'invoices' && (
            <aside className="cart" style={{width: 380, borderLeft:'1px solid var(--brd)', background:'var(--panel)', display:'flex', flexDirection:'column'}}>
      ...
      {toast && <div className="toast" style={{position:'fixed', top:20, right:20, background: toast.s === 'error' ? '#ef4444' : 'var(--p)', padding:'15px 30px', borderRadius:12, zIndex:2000}}><b>{toast.t.toUpperCase()}</b> : {toast.m}</div>}
    </div>
  );
}
