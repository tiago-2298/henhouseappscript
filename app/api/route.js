...
const SHEETS_TIMEOUT_MS = 15000; // 15s pour éviter de bloquer Vercel
...
async function getAuthSheets() {
  ...
  google.options({ timeout: SHEETS_TIMEOUT_MS, retry: false });
...
async function updateEmployeeStats(employeeName, amount, type) {
  try {
    ...
    const rowIndex = rows.findIndex(r => r[0]?.trim().toLowerCase() === employeeName.trim().toLowerCase());
...
export async function POST(request) {
  try {
    ...
    if (!action || action === 'getMeta' || action === 'syncData') {
      ...
      if (cache?.meta && (now - cache.ts) < CACHE_TTL_MS) return NextResponse.json(cache.meta);
    ...
    switch (action) {
      case 'sendFactures':
        ...
        embed.fields = [
          ...
          { name: '📋 Items', value: data.items?.map(i => `🔸 x${i.qty} ${i.desc}`).join('\n') }
        ];
      ...
      case 'sendProduction':
        ...
        embed.fields = [
          { name: '📊 Total', value: `${tProd} u.`, inline: true },
      ...
      case 'sendEntreprise':
        embed.title = `🚚 Livraison Pro : ${data.employee}`;
        embed.fields = [
          { name: '🏢 Client', value: data.company },
      ...
      case 'sendExpense':
        ...
        embed.fields = [
          ...
          { name: '🚗 Véhicule', value: data.vehicle || 'N/A' }
        ];
      ...
      case 'sendGarage':
        embed.title = data.action === 'Sortie' ? `🔑 Sortie : ${data.employee}` : `🅿️ Entrée : ${data.employee}`;
      ...
      case 'sendPartnerOrder':
        embed.title = `🤝 Partenaire : ${data.employee}`;
        embed.fields = [
          { name: '🏢 Entrep.', value: data.company, inline: true },
      ...
      case 'sendSupport':
        embed.title = `🆘 Ticket Support : ${data.employee}`;
        embed.description = `**Sujet:** ${data.sub}\n**Message:**\n${data.msg}`;
