...
async function updateEmployeeStats(employeeName, amountToAdd, type) {
  try {
    ...
    // On lit les noms pour trouver la ligne
    ...
    const rowIndex = rows.findIndex(r => r[0] && String(r[0]).trim() === String(employeeName).trim());
    if (rowIndex === -1) return;

    const realRow = rowIndex + 2; // car B2 => index 0
    const targetCell = type === 'CA'
      ? `'Employés'!G${realRow}`
      : `'Employés'!H${realRow}`;
    ...
    currentValue = toNum(currentValue);

    const newValue = currentValue + toNum(amountToAdd);
...
async function getEmployeesNamesOnly() {
  try {
...
export async function POST(request) {
  try {
    ...
    if (!action || action === 'getMeta') {
      const employees = await getEmployeesNamesOnly();
    ...
    if (action === 'getDirectory') {
      ...
      return NextResponse.json({ success:true, employeesFull });
    }

    // ===== LEADERBOARD =====
    if (action === 'getLeaderboard') {
      const employeesFull = await getEmployeesFullFromGoogle();
      const topCA = [...employeesFull].sort((a,b)=>b.ca-a.ca).slice(0, 5);
      const topStock = [...employeesFull].sort((a,b)=>b.stock-a.stock).slice(0, 5);
      return NextResponse.json({ success:true, topCA, topStock });
    }
