...
const WEBHOOKS = {
  ...
  entreprise:
    "https://discord.com/api/webhooks/1389356140957274112/6AcD2wMTkn9_1lnZNpm4fOsXxGk0sZR5us-rWSrbdTBScu6JYbMtWi31No6wbepeg607",
  garage:
...
// ✅ lecture + parsing employés complet (A..I)
async function getEmployeesFullFromGoogle() {
  try {
    const sheets = await getAuthSheets();
    const sheetId = process.env.GOOGLE_SHEET_ID;

    // Ton onglet semble être la feuille 1, colonnes A..I
    ...
    const employeesFull = rows
      .filter((r) => r[1] && String(r[1]).trim() !== "")
      .map((r) => ({
        id: r[0] ?? "",
        name: String(r[1] ?? "").trim(),
        poste: String(r[2] ?? "").trim(),
        telephone: String(r[3] ?? "").trim(),
        dateArrivee: r[4] ?? "",
        anciennete: Number(r[5] ?? 0) || 0,
        ca: Number(r[6] ?? 0) || 0,
        stock: Number(r[7] ?? 0) || 0,
        salaire: Number(r[8] ?? 0) || 0,
      }));

    employeesFull.sort((a, b) => a.name.localeCompare(b.name, "fr"));
    return employeesFull;
  } catch (e) {
    console.error("Erreur getEmployeesFullFromGoogle:", e);
    return [];
  }
}

async function getEmployeesFromGoogle() {
  const full = await getEmployeesFullFromGoogle();
  return full.map((e) => e.name);
}
...
async function updateEmployeeStats(employeeName, amountToAdd, type) {
  try {
    ...
    const rowIndex = rows.findIndex(
      (r) => r[0] && String(r[0]).trim() === String(employeeName).trim()
    );
    ...
    const newValue = Number(currentValue) + Number(amountToAdd || 0);
