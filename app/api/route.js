...
async function updateEmployeeStats(employeeName, amountToAdd, type) {
    try {
        ...
        // On récupère la liste des noms en colonne B
        ...
        if (rowIndex === -1) {
            console.warn(`Employé non trouvé: ${employeeName}`);
            return;
        }

        const realRow = rowIndex + 2;
        // G pour CA, H pour Stock (à adapter selon votre tableau)
        const targetColumn = type === 'CA' ? 'G' : 'H';
        const range = `'Employés'!${targetColumn}${realRow}`;
...
export async function POST(request) {
    try {
        ...
        // --- SYNC / INIT ---
        if (action === 'getMeta' || action === 'syncData') {
            let employeesFull = [];
            try {
                const sheets = await getAuthSheets();
        ...
        // --- LOGIQUE EMBED DISCORD PAR DÉFAUT ---
        let embed = {
            timestamp: new Date().toISOString(),
            footer: { text: `Hen House System v${APP_VERSION}` },
            color: 5814783 // Couleur violette par défaut
        };

        switch (action) {
            case 'sendFactures':
                const grandTotal = data.items.reduce((acc, i) => acc + (Number(i.qty) * (PRICE_LIST[i.desc] || 0)), 0);
                embed.title = `🍽️ Nouvelle Facture : N°${data.invoiceNumber || '???'}`;
                embed.fields = [
            ...
            case 'sendProduction':
                ...
                embed.fields = [
                    { name: '👤 Employé', value: data.employee, inline: true },
                    { name: '📊 Total Produits', value: `**${totalProd}** unités`, inline: true },
                    { name: '📝 Inventaire', value: data.items.map(i => `• ${i.product} : ${i.qty}`).join('\n') }
                ];
            ...
            case 'sendEntreprise':
                embed.title = '🏢 Commande B2B / Entreprise';
                embed.color = 3447003; // Bleu
                embed.fields = [
                    { name: '👤 Employé', value: data.employee, inline: true },
                    { name: '🏢 Société cliente', value: data.company, inline: true },
                    { name: '📋 Liste', value: data.items.map(i => `• ${i.product} x${i.qty}`).join('\n') }
                ];
                await sendWebhook(WEBHOOKS.entreprise, { embeds: [embed] });
                break;

            case 'sendGarage':
                embed.title = `🚗 Mouvement Garage : ${data.action}`;
                embed.fields = [
                    { name: '👤 Employé', value: data.employee, inline: true },
                    { name: '🚗 Véhicule', value: data.vehicle, inline: true },
                    { name: '⛽ Niveau Essence', value: `${data.fuel}%`, inline: true }
                ];
            ...
            case 'sendPartnerOrder':
                ...
                // Envoi au webhook spécifique de la boîte si existant, sinon webhook général
                const targetWebhook = PARTNERS.companies[data.company]?.webhook || WEBHOOKS.factures;
                await sendWebhook(targetWebhook, { embeds: [embed] });
                break;

            case 'sendSupport':
                embed.title = `🆘 Nouveau Ticket Support : ${data.subject}`;
                embed.description = `**Message :**\n${data.message}`;
                embed.fields = [{ name: '👤 De', value: data.employee }];
